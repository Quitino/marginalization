...
int main() {
  ...
  Eigen::Vector3d Wp = T_WC0.matrix().topLeftCorner(3, 3) * C0p +
                       T_WC0.matrix().topRightCorner(3, 1);
  ...
  Eigen::Matrix<double, 2, 6, Eigen::RowMajor> jacobian0_min; // 对第i帧的位姿 pi,qi求导
  Eigen::Matrix<double, 2, 3, Eigen::RowMajor> jacobian1_min; // 对第j帧的位姿 pj 求导
  ...
  projectFactor->EvaluateWithMinimalJacobians(paramters, residual.data(),
  ...
  // 解析求导
  projectFactor->EvaluateWithMinimalJacobians(
  ...
  num_differ.df_r_xi<2, 7, 6, PoseLocalParameterization>(
      parameters_noised, 0, num_jacobian0_min.data());

  std::cout << "\n\n\nanalytic_jacobian0_min:\n " << jacobian0_min << std::endl;
  std::cout << "numeric_jacobian0_min:\n "
            << num_jacobian0_min << std::endl;

  std::cout << "\n\n\nCheck jacobian0: "
            << num_differ.isJacobianEqual<2, 6>(
  ...
  std::cout << "\n\n\nanalytic_jacobian1_min:\n " << jacobian1_min << std::endl;
  std::cout << "numeric_jacobian1_min:\n "
            << num_jacobian1_min << std::endl;
  std::cout << "Check jacobian1: "
            << num_differ.isJacobianEqual<2, 3>(
