...
class NumDiff {
 public:
  NumDiff(Functor* ptrErrorFunctor) : ptrErrorFunctor_(ptrErrorFunctor) {
  ...
  template <int ResidualDim, /*  dim of residual*/
            int ParamDim, int MinimalParamDim, typename LoaclPrameter>
  bool df_r_xi(double** parameters, /* full parameters*/
               unsigned int paramId, double* jacobiansMinimal) {
    std::shared_ptr<LoaclPrameter> ptrlocalParemeter(new LoaclPrameter);
    Eigen::Map<
        Eigen::Matrix<double, ResidualDim, MinimalParamDim, Eigen::RowMajor>>
        miniJacobian(jacobiansMinimal);
    ...
    for (unsigned int i = 0; i < MinimalParamDim; i++) {
      ...
      applyDistribance(parameters, xi_plus_delta.data(), parameter_plus,
                       paramId);
      ...
      applyDistribance(parameters, xi_minus_delta.data(), parameter_minus,
                       paramId);
  ...
  template <int ResidualDim, int ParamDim>
  bool df_r_xi(double** parameters, unsigned int paramId, double* jacobian) {
    typedef Eigen::Matrix<double, ResidualDim, ParamDim,
                          (ParamDim > 1 ? Eigen::RowMajor : Eigen::ColMajor)>
        JocabinType;
   ...
   */
  template <int ResidualDim, int ParamDim>
  static bool isJacobianEqual(double* Jacobian_a, double* Jacobian_b,
                              double relTol = 1e-4) {
    Eigen::Map<Eigen::Matrix<double, ResidualDim, ParamDim, Eigen::RowMajor>>
 ...
 private:
   ...
   * @param ith
   */
  void applyDistribance(double** parameters, double* parameter_i,
